...
def test_scatter():
    for nvecs in [2, 10]:
        ...
        X = scatter(mask, target1, target2, a, add=False, complement=True)
        assert np.all(X[1::2, :] == a)
        X = scatter(mask, target1, target2, a, add=True, complement=True)
        X = scatter(mask, target1, target2, a, add=True, complement=True)
        assert np.all(X[1::2, :] == 3 * a)
        X = scatter(mask, target1, target2, b, add=False, complement=False)
        assert np.all(X[::2, :] == b)
        X = scatter(mask, target1, target2, b, add=True, complement=False)
